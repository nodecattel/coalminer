...
coal_address = "E3yUqBNTZxV8ELvW99oRLC7z4ddbJqqR4NphwrMug9zu"
...
def get_coal_rewards():
    try:
        result = subprocess.run(['coal', 'rewards'], capture_output=True, text=True)
...
def simulate_one_day(difficulty_levels, probabilities, rewards, coal_price, sol_price, fees_lamports, electric_cost, multiplier):
    fee_sol = (fees_lamports + 5000) / 1e9
    fee_usd = lambda: fee_sol * sol_price + electric_cost / 60
    total_coal, total_profit = 0, 0
    ...
    for _ in range(1440):  # 1440 minutes in a day
        ...
        total_coal += reward
        total_profit += reward * coal_price - fee_usd()
        difficulty_hits[difficulty] += 1

    return total_coal, total_profit, difficulty_hits
...
def display_tiered_summary(hits, rewards, total_passes, multiplier):
    ...
    for difficulty in sorted(hits):
        ...
        summary_data.append([
            difficulty,
            f"{rewards.get(difficulty, 0) * multiplier:.8f} COAL/pass",
            hits[difficulty],
            f"{percentage:.1f}%",
            f"{cumulative:.1f}%"
        ])
    print(tabulate(summary_data, headers=["Difficulty", "COAL Reward Rate", "Solves", "Percentage", "Cumulative"], tablefmt="pretty"))

def display_multiplier_preview(stake, top_stake, hits, rewards, coal_price):
    current_multiplier = multiplier.calculate_multiplier(stake, top_stake)
    print(f"{Fore.CYAN}\nMultiplier Preview:")
    print(f"Your Current Stake: {Fore.GREEN}{stake:.8f} COAL{Style.RESET_ALL}")
    ...
    for increment, new_multiplier in multiplier.preview_multipliers(stake, top_stake):
        ...
        expected_coal = sum(hits[difficulty] * rewards.get(difficulty, 0) * new_multiplier for difficulty in hits)
        preview_data.append([f"{stake + increment:.8f} COAL", f"{new_multiplier:.8f}",
                             f"{percentage_increase:.2f}%", f"${increment * coal_price:.2f}",
                             f"{expected_coal:.8f} COAL", f"${expected_coal * coal_price:.6f}"])

    print(tabulate(preview_data, headers=["Stake (COAL)", "Multiplier", "Increase (%)", "Cost of COAL Buy (USD)",
                                          "Expected COAL/Day", "Expected USD/Day"], tablefmt="pretty"))

def main():
    sol_price = get_token_price(sol_address)
    coal_price = get_token_price(coal_address)
    if not sol_price or not coal_price:
    ...
    rewards = get_coal_rewards()
    stake, top_stake = multiplier.get_stake_and_top_stake()
    multiplier_value = multiplier.calculate_multiplier(stake, top_stake)

    total_coal, total_profit, hits = simulate_one_day(difficulty_levels, probabilities, rewards, coal_price, sol_price,
                                                      priority_fees_lamports, electric_cost_per_hour, multiplier_value)

    print(f"SOL Price: {sol_price}, COAL Price: {coal_price}")
    ...
    display_multiplier_preview(stake, top_stake, hits, rewards, coal_price)

    fee_sol = priority_fees_lamports / 1e9
    print(f"{Fore.CYAN}\nCost Breakdown:")
    print(f"{Fore.YELLOW}SOL Price: {Style.RESET_ALL}${sol_price:.2f}")
    print(f"{Fore.YELLOW}COAL Price: {Style.RESET_ALL}${coal_price:.2f}")
    print(f"{Fore.YELLOW}Priority Fees: {Style.RESET_ALL}{fee_sol:.8f} SOL per transaction")
    print(f"{Fore.YELLOW}COAL Mining Fee: {Style.RESET_ALL}0.000005 SOL per transaction")
    print(f"{Fore.YELLOW}Electricity/Rental Fees: {Style.RESET_ALL}${electric_cost_per_hour:.2f} per hour")

    print(f"{Fore.CYAN}\nSummary:")
    print(f"Total COAL Mined for the Day: {Fore.GREEN}{total_coal:.8f} COAL{Style.RESET_ALL} | {Fore.GREEN}${total_coal * coal_price:.6f}")
    print(f"Expected COAL Mined for the Day (from probabilities): {Fore.GREEN}{total_coal:.8f} COAL")
