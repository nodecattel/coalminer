...
EOF
echo -e "Compatible with coal-cli v2.1.0 - Coal Miner V2"
echo -e "Made by NodeCattel & All the credits to Coal-Digital\033[0m\n"

# Configuration directory and file
COAL_DIR="$HOME/.coal"
CONFIG_FILE="$COAL_DIR/coal.conf"
...
# Ensure the COAL directory exists
mkdir -p "$COAL_DIR"
...
show_help() {
    $ECHO "A command line interface for the COAL cryptocurrency mining."
    $ECHO ""
    $ECHO "Usage: ./coal.sh <COMMAND>"
    $ECHO ""
    $ECHO -e "To solo mining run: \033[0;35m ./coal.sh mine\033[0m"
    $ECHO -e "To pool mining with Alvarium run: \033[0;35m ./coal.sh pool\033[0m"
    $ECHO ""
    $ECHO "Commands:"
    $ECHO "  balance    Fetch the Coal balance of an account"
    $ECHO "  benchmark  Benchmark your machine's hashrate"
    $ECHO "  busses     Fetch the bus account balances"
    $ECHO "  claim      Claim available mining rewards use --amount <COAL AMOUNT> to partial claim"
    $ECHO "  close      Close your onchain accounts to recover rent"
    $ECHO "  config     Fetch the program config"
    $ECHO "  mine       Start mining Coal"
    $ECHO "  pool       Join Alvarium Mining Pool"
    $ECHO "  rewards    Fetch the reward rate for each difficulty level"
    $ECHO "  stake      Stake coal to earn a multiplier on your mining rewards use --amount <COAL AMOUNT> to partial stake"
    $ECHO "  upgrade    Upgrade your COAL tokens from v1 to v2"
}
...
fetch_balance() {
    ...
    coal balance --rpc "$RPC" --keypair "$KEYPAIR_PATH"
}
...
fetch_busses() {
    ...
    coal busses --rpc "$RPC" --keypair "$KEYPAIR_PATH"
}
...
claim_rewards() {
    ...
    coal claim --rpc "$RPC" --keypair "$KEYPAIR_PATH" --priority-fee "$PRIORITY_FEE" $AMOUNT $RECEIVER
}
...
stake_coal() {
    ...
    $ECHO "Staking coal to earn a multiplier on your mining rewards..."
    coal stake --rpc "$RPC" --keypair "$KEYPAIR_PATH" --priority-fee "$PRIORITY_FEE" $AMOUNT $SENDER
}

# Function to close onchain accounts to recover rent
close_accounts() {
    $ECHO "Closing onchain accounts to recover rent..."
    coal close --rpc "$RPC" --keypair "$KEYPAIR_PATH"
}
...
# Function to upgrade COAL tokens from v1 to v2
upgrade_tokens() {
    $ECHO "Upgrading your COAL tokens from v1 to v2..."
    coal upgrade --rpc "$RPC" --keypair "$KEYPAIR_PATH"
}
...
run_mining() {
    while :; do
        ...
        if [[ "$JITO_ENABLED" == "true" ]]; then
            coal mine --rpc "$RPC" --keypair "$KEYPAIR_PATH" --priority-fee "$PRIORITY_FEE" --cores "$CORES" --buffer-time "$BUFFER_TIME" --jito
        elif [[ -n "$DYNAMIC_FEE_URL" ]]; then
            coal mine --rpc "$RPC" --keypair "$KEYPAIR_PATH" --cores "$CORES" --dynamic-fee-url "$DYNAMIC_FEE_URL" --dynamic-fee --buffer-time "$BUFFER_TIME"
        else
            coal mine --rpc "$RPC" --keypair "$KEYPAIR_PATH" --priority-fee "$PRIORITY_FEE" --cores "$CORES" --buffer-time "$BUFFER_TIME"
        fi
        if [[ $? -ne 0 ]]; then
            $ECHO "\033[0;35mMining process exited with error, restarting in 3 seconds...\033[0m"
            $SLEEP 3
        else
            $ECHO "\033[0;35mMining process completed successfully.\033[0m"
            break
        fi
    done
}
...
case "$COMMAND" in
    ...
    busses)
        ...
        ;;
    claim)
        $ECHO "Enter wallet address to receive COAL claim (or leave empty to use the default wallet): "
    ...
    upgrade)
        ...
        ;;
    mine)
        # Display Coal Cli version
        coal_version=$(coal --version)
        $ECHO "Cli Version: $coal_version"
 ...
 "$input_jito" =~ ^[Yy]$ ]]; then
        ...
        if [ ! -f "$KEYPAIR_PATH" ]; then
            $ECHO "\033[0;35mGenerating a new Solana keypair...\033[0m"
            $ECHO "\033[1;35mIMPORTANT: The next output will include your seed phrase. Please make sure to write it down and store it safely!\033[0m"
            read -p "Press enter to continue and see your seed phrase..."
            solana-keygen new --outfile "$KEYPAIR_PATH"
            $ECHO "\033[1;35mPlease make sure you have saved your seed phrase securely.\033[0m"
        ...
        $ECHO -e "You are mining to the wallet address: \033[1;35m${WALLET_ADDRESS}\033[0m"

        # Print the final command used
        if [[ "$JITO_ENABLED" == "true" ]]; then
            final_command="coal mine --rpc \"$RPC\" --keypair \"$KEYPAIR_PATH\" --priority-fee \"$PRIORITY_FEE\" --cores \"$CORES\" --buffer-time \"$BUFFER_TIME\" --jito"
        elif [[ -n "$DYNAMIC_FEE_URL" ]]; then
            final_command="coal mine --rpc \"$RPC\" --keypair \"$KEYPAIR_PATH\" --cores \"$CORES\" --dynamic-fee-url \"$DYNAMIC_FEE_URL\" --dynamic-fee --buffer-time \"$BUFFER_TIME\""
        else
            final_command="coal mine --rpc \"$RPC\" --keypair \"$KEYPAIR_PATH\" --priority-fee \"$PRIORITY_FEE\" --cores \"$CORES\" --buffer-time \"$BUFFER_TIME\""
        fi

        $ECHO -e "Final command: \033[1;35m$final_command\033[0m"

        read -p "Press any key to start mining or CTRL+C to cancel..."

        # Display configuration
        $ECHO -e "\033[0;35mStarting mining operation with the following configuration:\033[0m"
        $ECHO "Cli Version: $coal_version"
        $ECHO "RPC URL: $RPC"
